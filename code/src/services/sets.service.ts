...
export class SetsService implements ISetsService {
  ...
  async createSetFromList(listId: UUID, data: CreateSetDTO): Promise<ShoppingSet> {
    // 1. Fetch List Items
  ...
  async updateSetItem(itemId: UUID, data: Partial<CreateSetItemDTO>): Promise<SetItem> {
